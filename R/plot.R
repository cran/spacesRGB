...
plotPatchesRGB  <-  function( obj, space='sRGB', which='signal', maxColorValue=1,
    ...
    if( is.matrix(obj) )
        ...
        if( ncol(obj) != 3 )
            {
            log.string( ERROR, "'%s' has %d columns, but it must have 3.", deparse(substitute(obj)), ncol(obj) )
            return(FALSE)
            }    
        ...
        if( ! is.null(colnames(obj)) )
            ...
            if( ! ok )
                log.string( WARN, "The column names of matrix obj are '%s', and do not look like RGB.", 
    ...
    if( ! is.data.frame(obj) )
        {
        log.string( ERROR, "obj is invalid; neither matrix nor data.frame" )
        return(FALSE)
        }
    ...
    if( ! ok )
        {
        log.string( ERROR, "maxColorValue='%s' is invalid.", as.character(maxColorValue[1]) )
        return(FALSE)
        }
    ...
    if( ! add )
        ...
        if( is.numeric(background) )
            ...
            if( length(background) != 3 )
                {
                log.string( ERROR, "background is invalid, because length(background)==%d is not 1 or 3.", length(background) )
                return(FALSE)
                }
